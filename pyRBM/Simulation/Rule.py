...
class Rule:
        ...
        if isinstance(propensity, (list)):
            ...
            for slot_i, formula_str in enumerate(propensity):
                ...
                if "loc_" not in formula_str:
                    ...
                    f = {loc_index: sympy.lambdify(formula_symbols,
                                                   sympy.parse_expr(self._subsituteConstants(formula_str, locations[loc_index].location_constants)).simplify(),
                                                   "numpy")
                        for loc_index in applicable_indices}

                    self.sympy_formula.append(sympy.parse_expr(self._subsituteConstants(formula_str,
                                                locations[applicable_indices[0]].location_constants)))
    ...
    def _subsituteConstants(self, formula_str:str, location_constants:dict) -> str:
        out_formula = formula_str
        for loc_constant in location_constants:
            out_formula = out_formula.replace(loc_constant,
                                              str(location_constants[loc_constant]))
        return out_formula
    ...
    def _locationAttemptedCompartmentChange(self, class_values, location_index:int,
                                            times_triggered:int):
    ...
    def returnPropensity(self, locations, builtin_classes):
        ...
        for loc_i, location in enumerate(locations):
            ...
            if not self.contains_location_constant[loc_i]:
                ...
                propensity *= max(0, self.lambda_propensities[loc_i][location.index](*location.class_values,
                                                                                     *builtin_classes))
    ...
    def triggerAttemptedRuleChange(self, locations,
        ...
        for loc_i, location in enumerate(locations):
            new_location_values = self._locationAttemptedCompartmentChange(location.class_values,
                                                                           loc_i, times_triggered)
            # CHANGE TODO
